...
For the first run, start small: one keyword or Page ID, one country, `adStatus` set to `active`, and `maxResults` between 5 and 10.
...
- `impressionsRange` — viewership range, when publicly disclosed
- `spendRange` — estimated spend, when publicly disclosed
- `countriesRunningIn` — target countries
- `languages` — ad languages
- `platformsList` — where the ad runs
- `fundingEntity` — funding entity for political/issue ads
- `paidForByText` — "Paid for by" disclosure
- `targetingInfo` — age, gender, location when publicly disclosed
- `adLibraryUrl` — direct link to the ad in the library
- `searchQuery` — original search keyword or page query
...
2. **Creative intelligence** — analyze ad copy, headlines, CTAs, and image/video formats across your industry.
3. **Political ad monitoring** — track political advertising spend, funding entities, disclosures, and targeting where Meta makes it public.
4. **Spend and impressions benchmarking** — pull spend ranges and impression ranges when available.
...
This Actor uses Apify Pay Per Event + platform usage pricing. You pay for ad records successfully saved to the dataset, and you also pay the Apify platform usage generated by the run, such as compute and residential proxy traffic.

| Event name | Price per event | 1,000 ads | 10,000 ads |
| --- | ---: | ---: | ---: |
| `ad-scraped` | $0.001 | $1.00 | $10.00 |

Failed, blocked, duplicate, or empty results are not billed as ad-scraped events.

Cost-control tips:

- Start with one keyword, advertiser name, or Page ID.
- Use one country such as `US` instead of `ALL` for the first test.
- Keep `adStatus` set to `active` unless you need inactive ads.
...
| `country` | string | no | `US` | ISO 3166-1 alpha-2 code (e.g. US, GB, DE) or ALL. |
...
| `maxResults` | integer | no | `10` | Max ads per search query. Use 5-10 for tests; up to 1000. |
...
2. Enter one search keyword, advertiser name, or numeric Page ID.
3. Set a `country`, keep `adStatus` as `active`, and choose platforms if needed.
4. For a first run, set `maxResults` to 5-10.
5. Run, then export the results as JSON, CSV, Excel, or HTML, or pull them via the Apify API.

## Output dataset

The default **Scraped Ads** dataset view is export-ready for competitor ad research. It keeps the key fields visible first: ad ID, advertiser, page ID, ad text, headline, description, creative type, impressions, spend, start/end dates, search query, and scraped timestamp. The full JSON output can also include advertiser URLs, destination URLs, images, videos, platform lists, countries, language, funding disclosures, and targeting information when Facebook publicly exposes those fields.
